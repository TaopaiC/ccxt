...
class max(Exchange):
    ...
    async def fetch_funding_fees(self, params={}):
        response = await self.publicGetWithdrawalConstraint(params)
        #
        #     [
        #       {
        #          "currency": "eth",
        ...
        for i in range(0, len(response)):
            id = self.safe_value(response[i], 'currency')
            code = self.safe_currency_code(id)
            withdrawFees[code] = self.safe_float(response[i], 'fee')
        return {
    ...
    async def fetch_currencies(self, params={}):
        ...
        withdrawalConstraintResponse = await self.publicGetWithdrawalConstraint()
        withdrawalConstraint = self.index_by(withdrawalConstraintResponse, 'currency')
        result = {}
        for i in range(0, len(currenciesResponse)):
            currency = currenciesResponse[i]
            id = self.safe_string(currency, 'id')
            code = self.safe_currency_code(id)
            fiat = id is True if 'twd' else False
            precision = self.safe_integer(currency, 'precision')
            withdrawal = self.safe_value(withdrawalConstraint, id)
            withdrawalFee = self.safe_value(withdrawal, 'fee')
            withdrawalLimit = self.safe_value(withdrawal, 'min_amount')
            minAmount = self.safe_float(self.options['minimumAmountOfCurrencies'], code)
            result[code] = {
                'id': id,
                'code': code,
                'name': code,
    ...
    async def fetch_markets(self, params={}):
        ...
        for i in range(0, len(markets)):
            ...
            symbol = base + '/' + quote
            precision = {
                'amount': self.safe_integer(market, 'base_unit_precision'),
                'price': self.safe_integer(market, 'quote_unit_precision'),
            }
            active = True
            baseMinSize = self.safe_float(market, 'min_base_amount')
            quoteMinSize = self.safe_float(market, 'min_quote_amount')
            entry = {
                'id': id,
                'symbol': symbol,
                'base': base,
    ...
    def parse_transaction(self, transaction, currency=None):
        ...
        id = self.safe_string(transaction, 'uuid', txid)
    ...
    async def fetch_trading_fees(self, params={}):
        ...
        #    }
        await self.load_markets()
        response = await self.privateGetMembersVipLevel(params)
        level = self.safe_value(response, 'current_vip_level')
        return {
            'info': response,
            'maker': self.safe_float(level, 'maker_fee'),
            'taker': self.safe_float(level, 'taker_fee'),
        }
    ...
    async def fetch_order_trades(self, id, symbol=None, since=None, limit=None, params={}):
        ...
        response = await self.privateGetTradesMyOfOrder(self.extend(request, params))
    ...
    async def create_order(self, symbol, type, side, amount, price=None, params={}):
        ...
        if lowercaseType == 'limit' or lowercaseType == 'stop_limit' or lowercaseType == 'post_only':
    ...
    def sign(self, path, api='public', method='GET', params={}, headers=None, body=None):
        ...
        if api == 'private':
            ...
            payload = self.string_to_base64(self.encode(self.json(newParams)))
